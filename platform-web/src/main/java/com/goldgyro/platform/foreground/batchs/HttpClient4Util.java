...
public class HttpClient4Util {
     ...
     * @return
     */
    public String post(String url, String code, Map<String, String> paramsMap) {
        String body = null;
        ...
        try {
        	for(String key: paramsMap.keySet()) {
        		paramList.add(new BasicNameValuePair(key, (String)paramsMap.get(key)));
        		System.out.print("请求参数======="+key+":"+paramsMap.get(key));
        	}
        	
            // Post请求
        	System.out.print("请求地址======="+(url+code));
            httpPost = new HttpPost(url+code);
	...
	public byte[] doPost(String url, Map<String, String> queryParams, Map<String, String> formParams) {
		System.out.print("请求报文：==="+JSONObject.toJSONString(formParams));
		//System.out.print("请求头报文：==="+queryParams.toString());
		if (url.isEmpty()) {
			return null;
		}
		CloseableHttpResponse response = null;
		...
		try {
			...
			if (queryParams != null && !queryParams.isEmpty()) {
				...
				for (String key : queryParams.keySet()) {
					...
					//我添加的
					httppost.addHeader(key, queryParams.get(key));
				}
				builder.setParameters(params);
			}
			//demo所实现的
			...
			int stateCode = response.getStatusLine().getStatusCode();
			System.out.print("cade===:"+stateCode);
			if (200 != stateCode) {
				//logger.error("非正常响应[" + stateCode + "]", new String(EntityUtils.toByteArray(entity), "UTF-8"));
				System.out.print("非正常响应[" + stateCode + "]"+ new String(EntityUtils.toByteArray(entity), "UTF-8"));
				return null;
			}
			if (entity != null) {
				return EntityUtils.toByteArray(entity);
